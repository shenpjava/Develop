...
##### 不安全的扫描类型
- 由于页拆分,扫描可能多次返回某些行或忽略某些行的情况.
- 满足以下条件时,存储引擎将采用不安全的分配顺序扫描:
  - 索引大小超过64页
  - 执行计划显示Index Scan,Ordered:False
  - 查询在未提交读的隔离级别下运行
  - 允许对读取的数据进行修改
...
- 检查索引的碎片级别
  ...
  (
  	DB_ID('Performance'),		--Performance:服务器名
  	OBJECT_ID('dbo.T1'),		--dbo.T1:表名
  	1,
  	NULL,
  	NULL
  )
